...
from ..splits import load_or_build_splits
...
def run(cfg: dict, paths: RunPaths, device: torch.device, *, resume: bool = False) -> None:
    ...
    if len(dataset) < 3:
        print(f"[vit] need >=3 *_spatial.pt files in {paths.features}. Run extract_features first.")
        return
    ...
    train_idx = smoke_trim(indices_from(dataset.samples, splits, "train"), cfg)
    val_idx = smoke_trim(indices_from(dataset.samples, splits, "val"), cfg)
    if not train_idx or not val_idx:
        print("[vit] Error: empty train or val split.")
        return
    ...
    sanity.preflight(stage="train_vit", model=model, dataset=dataset, splits=splits,
                     train_loader=train_loader, forward_fn=vit_forward, device=device,
                     ckpt_dir=paths.checkpoints, ckpt_prefix="vit")
