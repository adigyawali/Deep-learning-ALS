...
# Spatial CNN feature maps (output of generate_spatial_features.py).
# Each *.pt file contains (C, D', H', W') tensors for T1/T2/FLAIR.
# Override in Colab with env var: ALS_CNN_SPATIAL_FEATURES_DIR
SPATIAL_FEATURES_DIR = Path(
    os.environ.get(
        "ALS_CNN_SPATIAL_FEATURES_DIR",
        str((MODULE_DIR / "../cnn_features/cnn_multimodal/spatial_features").resolve()),
    )
).resolve()

# ViT artifacts are kept separate from CNN artifacts.
# Override in Colab with env var: ALS_VIT_ARTIFACTS_DIR
ARTIFACTS_DIR = Path(
    os.environ.get(
        "ALS_VIT_ARTIFACTS_DIR",
        str((MODULE_DIR / "../cnn_features/vit_multimodal").resolve()),
    )
).resolve()
CHECKPOINT_DIR = ARTIFACTS_DIR / "checkpoints"
METRICS_DIR = ARTIFACTS_DIR / "metrics"

CHECKPOINT_PATH = CHECKPOINT_DIR / "vit_spatial_best.pth"
...
def build_runtime_paths(features_dir: str | None = None, artifacts_dir: str | None = None):
    ...
    """
    runtime_features_dir = Path(features_dir).resolve() if features_dir else SPATIAL_FEATURES_DIR
    ...
    runtime_checkpoint_path = runtime_checkpoint_dir / "vit_spatial_best.pth"
